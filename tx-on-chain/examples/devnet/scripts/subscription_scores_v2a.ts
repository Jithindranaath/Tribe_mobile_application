...
async function main() {
  ...
  try {
    ...
    const strategy1PlusTo0: NDimensionalStrategy = {
      geometricTargets: [],
      distancePredicate: null,
      discretePredicates: [
        { single: { index: 0, predicate: { threshold: 0, comparison: { greaterThan: {} } } } },
        { single: { index: 1, predicate: { threshold: 0, comparison: { equalTo: {} } } } }
      ]
    };
    ...
    try {
      ...
      console.log(`[${name}] Executing 1+:0 discrete validation`);
      
      const isValid1To3Plus = await userProgram.methods
        .validateStatV2(payload2Leg, strategy1PlusTo0)
        .accounts({
      ...
      if (isValid1To3Plus) {
        console.log(`[${name}] 1+:0 validation passed`);
      } else {
        console.log(`[${name}] 1+:0 validation rejected`);
      }

      console.log(`[${name}] Executing binary draw validation`);
      
      const isValidDraw = await userProgram.methods
        .validateStatV2(payload2Leg, strategyDraw)
        .accounts({
          dailyScoresMerkleRoots: dailyScoresPda,
        })
        .preInstructions([computeBudgetIx])
      ...
      console.log(`[${name}] Executing combined 3-leg validation`);

      const isValid3Leg = await userProgram.methods
        .validateStatV2(payload3Leg, strategy3Leg)
        .accounts({
          dailyScoresMerkleRoots: dailyScoresPda,
        })
        .preInstructions([computeBudgetIx])
      ...
      if (isValid3Leg) {
        console.log(`[${name}] Combined 3-leg validation passed`);
      } else {
        console.log(`[${name}] Combined 3-leg validation rejected`);
      }

      // This validation does not fit into the MTU of 1000 bytes.
      // Run or study subscription_scores_v3c to use the V3 stat validation.
      // console.log(`[${name}] Executing combined 4-leg validation`);

      // const isValid4Leg = await userProgram.methods
      //   .validateStatV2(payload, strategy4Leg)
      //   .accounts({
      //     dailyScoresMerkleRoots: dailyScoresPda,
      //   })
      //   .preInstructions([computeBudgetIx])
      ...
      // if (isValid4Leg) {
      //   console.log(`[${name}] Combined 4-leg validation passed`);
      // } else {
      //   console.log(`[${name}] Combined 4-leg validation rejected`);
      // }

      const isValid2LegGeometric = await userProgram.methods
        .validateStatV2(payload2Leg, strategyGeometric)
        .accounts({
          dailyScoresMerkleRoots: dailyScoresPda,
        })
        .preInstructions([computeBudgetIx])
